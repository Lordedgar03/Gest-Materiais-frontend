...
import {
  FileText, PlusCircle, AlertCircle, Search, PackagePlus,
  CheckCircle, XCircle, Trash2, Undo2, X, Eye, MoreHorizontal, ChevronDown
} from "lucide-react"
...
function ReqCard({
  ...
  if (firstItem) {
    ...
    canAtender  = restante > 0 && mayOperate
    canDevolver = emUso > 0 && isAprovadorDaRequisicao(req)
  }

  const canApprove   = req.req_status === "Pendente" && mayDecide
  const canReject    = req.req_status === "Pendente" && mayDecide
  const canCancel    = req.req_status === "Pendente" && mayDecide
  const canDeleteBtn = isOwner || mayDecide

  const actions = [
    { key: "view",    label: "Ver",           icon: Eye,        onClick: () => openView(req),                        style: "border-blue-300 bg-blue-600 text-white hover:bg-blue-700 focus-visible:ring-blue-400" },
    { key: "approve", label: "Aprovar",       icon: CheckCircle,onClick: () => openDecision(req, "Aprovar"),         style: "border-emerald-300 bg-emerald-600 text-white hover:bg-emerald-700 focus-visible:ring-emerald-400", show: canApprove },
    { key: "reject",  label: "Rejeitar",      icon: XCircle,    onClick: () => openDecision(req, "Rejeitar"),        style: "border-rose-300 bg-rose-600 text-white hover:bg-rose-700 focus-visible:ring-rose-400", show: canReject },
    { key: "cancel",  label: "Cancelar",      icon: X,          onClick: () => openDecision(req, "Cancelar"),        style: "border-amber-300 bg-amber-600 text-white hover:bg-amber-700 focus-visible:ring-amber-400", show: canCancel },
    { key: "serve1",  label: "Atender 1º",    icon: CheckCircle,onClick: () => openAtender(req, firstItem),          style: "border-indigo-300 bg-indigo-600 text-white hover:bg-indigo-700 focus-visible:ring-indigo-400", show: canAtender },
    { key: "return1", label: "Devolver 1º",   icon: Undo2,      onClick: () => openDevolver(req, firstItem),         style: "border-teal-300 bg-teal-600 text-white hover:bg-teal-700 focus-visible:ring-teal-400", show: canDevolver },
    { key: "delete",  label: "Excluir",       icon: Trash2,     onClick: () => openDelete(req.req_id),               style: "border-gray-300 bg-white text-gray-800 hover:bg-gray-50 focus-visible:ring-gray-400", show: canDeleteBtn },
  ].filter(a => a.show === undefined ? true : a.show)

  // Em telas pequenas, mostramos "Ver" + botão "Mais"
  const primaryMobile = actions[0] // “Ver”
      ...
      <div className="p-4">
        {firstItem ? (
          <div className="rounded-lg border bg-gray-50 p-3">
            <div className="text-sm font-medium text-gray-900 line-clamp-1">{materialNome(firstItem.rqi_fk_material)}</div>
          ...
          <div className="rounded-lg border bg-gray-50 p-3 text-sm text-gray-600">Sem itens nesta requisição.</div>
        )}
      </div>
            ...
            {restMobile.length > 0 && (
              <div className="relative">
                ...
                {moreOpen && (
                  <div
                    role="menu"
                    className="absolute right-0 mt-2 min-w-[12rem] rounded-lg border bg-white shadow-lg z-10 p-2"
                  >
                    {restMobile.map(a => (
                      <button
                        key={a.key}
...
export default function Requisicao() {
  const {
    // identidade/perms
    currentUser, isAprovadorDaRequisicao,
    // dados
    materiais, filtered,
    // ui base
  ...
  const list = useMemo(() => {
    ...
    return filtered.filter(req => {
      ...
      const matchItem = Array.isArray(req.itens) && req.itens.some(it => (materialNome(it.rqi_fk_material) || "").toLowerCase().includes(term))
  ...
  return (
    <main className="h-[100svh] bg-gradient-to-b ">
      <div className="h-full flex flex-col">

        {/* TOPO FIXO: header + erro + filtros */}
          ...
          <div className=" mx-auto">
            ...
            <header className="px-4 sm:px-6 py-4 sm:py-5 rounded-lg">
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  <PlusCircle size={18} aria-hidden /> <span className="hidden sm:inline">Nova requisição</span><span className="sm:hidden">Novo</span>
                </button>
              </div>
            </header>
            ...
            {error && (
              <div role="alert" aria-live="assertive" className="border-y bg-rose-50 border-rose-200 text-rose-800">
                <div className="px-4 sm:px-6 py-3 flex items-center gap-2">
                  ...
                  <button
                    className="ml-auto underline decoration-rose-500 hover:text-rose-900"
            ...
            <section className="px-4 sm:px-6 py-3 sm:py-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                ...
                <div className="md:col-span-2">
                  <label htmlFor="busca-req" className="block text-sm text-gray-700 mb-1">Pesquisar (código ou material)</label>
                  <div className="relative">
        ...
        {/* ÁREA ROLÁVEL — grid responsivo com auto-fit/minmax */}
      ...
      <Modal
        ...
        footer={
          <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
            ...
            >
              {submitting && <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-30" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-90" fill="currentColor" d="M4 12a8 8 0 018-8v4A4 4 0 008 12H4z"/></svg>}
              Enviar
            </button>
          </div>
        }
      >
        <form id="form-req" onSubmit={submitRequisicao} className="space-y-4">
          ...
          <div className="pt-3 border-t space-y-3">
            <h3 className="font-semibold flex items-center gap-2 text-gray-900"><PackagePlus size={16}/> Itens</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
              ...
              <div>
                ...
                <div className="font-medium">
                  {viewReq.req_date ? new Date(viewReq.req_date).toLocaleString() : "—"}
                </div>
              </div>
              <div>
                <div className="text-gray-500">Usuário</div>
                <div className="font-medium">
                  {viewReq.req_user_nome ?? viewReq.user_nome ?? (viewReq.req_fk_user ? `#${viewReq.req_fk_user}` : "—")}
                </div>
              </div>
            </div>
