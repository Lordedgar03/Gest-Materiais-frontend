  ...
  const handleExportExcel = async () => {
    try {
      ...
      const rows = data.map(r => ({
        ...
        "Preço (STN)": Number(r.preco).toFixed(2),
        "Total (STN)": Number(r.preco * r.quantidade).toFixed(2),
      ...
      const resumo1 = [
        ...
        { Métrica: "Entradas (STN)",    Valor: summary.entradas.toFixed(2) },
        { Métrica: "Saídas (STN)",      Valor: summary.saidas.toFixed(2) },
        { Métrica: "Balanço (STN)",     Valor: (summary.entradas - summary.saidas).toFixed(2) },
      ]
  ...
  return (
    <main className="min-h-screen dark:bg-gray-950 space-y-6 " aria-labelledby="reports-title">
              ...
              <table className="min-w-full text-sm ">
                ...
                <thead className="bg-gray-100/70 dark:bg-gray-800/60 border-gray-300">
                  <tr>
                    ...
                    <Th className="text-right"> Total (STN) </Th>
                    <Th> Requisição </Th>
                  </tr>
                </thead>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4  " aria-label="Resumo">
        ...
        <KPI label="Entradas (STN)" value={summary.entradas.toFixed(2)} />
        <KPI label="Saídas (STN)" value={summary.saidas.toFixed(2)} />
      </section>
    </main>
  )
}
