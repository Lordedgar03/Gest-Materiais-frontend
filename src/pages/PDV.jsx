...
/* ========== Impressão térmica (HTML do backend) ========== */
function useThermalPrint() {
  const [printing, setPrinting] = React.useState(false);

  const printBlob = React.useCallback((blob) => {
  ...
  // térmico (HTML) por venda
  const printReceiptBySaleId = React.useCallback(async (venId) => {
    setPrinting(true);
    try {
      const res = await api.request({
        url: `/vendas/${venId}/recibo/pdf`,
        method: "POST",
        responseType: "blob",
  ...
  // ✅ PDF por recibo (novo fallback)
  const printReciboPdfById = React.useCallback(async (recId) => {
    setPrinting(true);
    try {
      const res = await api.request({
        url: `/recibos/${recId}/pdf`,
        method: "GET",
        responseType: "blob",
  ...
  return { printing, printReceiptBySaleId, printReciboPdfById };
}
...
export default function PDV() {
  ...
  const { printing, printReceiptBySaleId } = useThermalPrint();
...
const handleCheckout = async () => {
  ...
  const recId =
    ...
    result?.paid?.recibo?.rec_id ||
    null;

  // Preferência: térmico por venda
  if (venId) {
    const okThermal = await printReceiptBySaleId(venId);
    if (okThermal) return;

    // fallback: PDF por recibo (se existir)
    if (recId) {
      const okPdf = await printReciboPdfById(recId);
      if (okPdf) return;
    }

    setToast("Venda feita, mas não foi possível imprimir o recibo.");
    return;
  }

  // Se não veio venId mas veio recId, ainda dá para imprimir PDF
  if (recId) {
    const okPdf = await printReciboPdfById(recId);
    if (!okPdf) setToast("Venda feita, mas não foi possível imprimir o recibo.");
  }
};
            ...
            <div className=" mx-auto px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {caixaAberto ? (
                  ...
                  <a
                    href="/caixa"
                    className="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-100 text-amber-800 dark:bg-red-400 dark:text-white"
                  >
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="font-medium text-gray-900 dark:text-zinc-100 line-clamp-2">
                              {p.nome}
                            </div>
                            <span
            ...
            <aside className="rounded-2xl border border-gray-200 shadow-sm p-5 bg-white dark:bg-zinc-900  dark:border-zinc-800 ">
              ...
              {values.cart.length === 0 ? (
                <div className="p-6 text-center text-gray-600 dark:text-zinc-300">
                  <Package
                    className="mx-auto text-gray-400 dark:text-zinc-500 mb-2"
                    size={40}
                  />
                    ...
                    {() =>
                      values.cart.map((it) => {
                        ...
                        return (
                          <div
                            key={it.id}
                            className="rounded-2xl border dark:border-zinc-800 p-3"
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div>
                            ...
                            <div className="mt-2 flex items-center justify-between">
                              <div className="inline-flex items-center rounded-lg border dark:border-zinc-800 dark:text-white">
                                <button
                                  type="button"
                                  onClick={() => dec(it.id)}
                                  className="p-1.5 hover:bg-gray-50 dark:hover:bg-zinc-800"
                                >
                                  <Minus size={14} />
                                </button>
                                <input
            ...
            <div className="space-y-4 text-gray-900 dark:text-white">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border rounded-lg px-3 py-2 bg-white dark:bg-zinc-950 dark:border-zinc-800 dark:text-white"
                >
                  <option value="Dinheiro">Dinheiro</option>
                  <option value="Multibanco">Multibanco</option>
                  <option value="Transferencia">Trânsferencia</option>
                  <option value="MB Way">MB Way</option>
                  <option value="Cartão">Cartão</option>
                </select>
                <p className="text-xs text-gray-600 dark:text-zinc-400 mt-1">
                  Esta informação será enviada no pagamento e pode aparecer no
                  recibo (se o backend usar o campo).
                </p>
              </div>
            </div>
          </Modal>
