...
export default function Vendas() {
  ...
  return (
    <main className="min-h-screen space-y-6 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-blue-900/20 dark:to-indigo-900/20">
      <header className="rounded-2xl p-6 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-200 dark:border-gray-700 shadow-lg">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-700 dark:text-gray-300">
                Filtros, exportação e acessibilidade aprimorada
              </p>
            </div>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-3 text-sm">
            ...
            <button
              className="btn btn-outline px-4 py-2 rounded-xl inline-flex items-center  bg-red-600 hover:bg-red-700 text-white font-semibold focus-visible:ring-2 ring-offset-2 ring-red-500"
              onClick={exportPdf}
              disabled={!allowed || loading || filtered.length === 0}
            >
              Exportar PDF
            </button>
          </div>
      ...
      <section className="rounded-2xl p-5 bg-white/90 dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700 backdrop-blur-xl shadow">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
          <div className="relative w-full md:w-80">
            <label htmlFor="busca-vendas" className="sr-only">
              Pesquisar vendas
            </label>
            <Search
              size={16}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold focus-visible:ring-2 ring-indigo-500"
            >
              Aplicar
            </button>
          </div>
          ...
          >
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 dark:border-gray-600 rounded-xl px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500"
              >
                {["Todos", "Aberta", "Paga", "Cancelada", "Estornada"].map(
                  (s) => (
            ...
            <div className="flex items-end">
              <button
                ...
                className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus-visible:ring-2 ring-indigo-500"
              >
                Limpar filtros
              </button>
            </div>
      ...
      {/* tabela */}
      <section
        className="rounded-2xl bg-white/95 dark:bg-gray-800/95 border border-gray-200 dark:border-gray-700 overflow-hidden backdrop-blur-xl shadow"
        role="region"
        aria-label="Tabela de vendas"
      >
        {loading ? (
          <div
            className="p-8 text-center text-gray-800 dark:text-gray-200"
            aria-live="polite"
          >
            Carregando…
          </div>
          ...
          <>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-slate-200 dark:bg-slate-700">
                  <tr>
                    <th
                      scope="col"
                      className="px-3 py-2 text-left font-bold text-slate-900 dark:text-slate-100"
                    >
                      Código
                    </th>
                    <th
                ...
                <tbody className="bg-white dark:bg-gray-800">
                  {current.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-gray-900 dark:text-gray-100">
                        {typeof s.itens_qtd === "number" ? s.itens_qtd : "—"}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <StatusBadge value={s.ven_status} />
                      </td>
                      <td className="px-3 py-2">
            ...
            <div className="bg-white/90 dark:bg-gray-800/90 px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <p className="text-sm text-gray-800 dark:text-gray-200">
                Mostrando <b>{(page - 1) * perPage + 1}</b>–
                <b>{Math.min(page * perPage, filtered.length)}</b> de{" "}
                <b>{filtered.length}</b>
              </p>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-gray-300 dark:border-gray-600 disabled:opacity-50 focus-visible:ring-2 ring-indigo-500"
                >
                  Anterior
                </button>
                <span
        ...
        footer={
          <div className="flex justify-end">
            <button
              ...
              className="px-4 py-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus-visible:ring-2 ring-indigo-500"
            >
              Fechar
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              ...
              <div>
                ...
                <div className="font-semibold">
                  <StatusBadge value={viewSale.ven_status} />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(viewSale.itens || viewSale.vendaItens || []).map(
                    (it, i) => (
                      <tr
                        key={i}
                      ...
                      >
                        <td className="px-3 py-2">
                          ...
                          {it.vni_material_nome ||
                            it.material_nome ||
                            it.nome ||
                            `#${it.vni_fk_material}`}
                        </td>
                        <td className="px-3 py-2">
                          {fmt(it.vni_preco_unit ?? it.preco_unit ?? it.preco)}
                        </td>
                        <td className="px-3 py-2 font-semibold">
                          {it.vni_qtd ?? it.qtd}
                        </td>
                        <td className="px-3 py-2">
                          {fmt(
                            (it.vni_preco_unit ?? it.preco_unit ?? it.preco) *
                              (it.vni_qtd ?? it.qtd)
                          )}
                        </td>
                      </tr>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={applyCancel}
              className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-semibold focus-visible:ring-2 ring-rose-500"
            >
              Confirmar
            </button>
          </div>
