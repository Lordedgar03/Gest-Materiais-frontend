...
import {
  ...
  X,
  BadgeInfo,
  UtensilsCrossed,   // Almoço (hub)
  GraduationCap,     // Alunos
  CalendarCheck2,    // Marcações
...
const TEMPLATE_TO_PERMS = {
  ...
  // gerir vendas (inclui PDV/Caixa/Vendas e todo módulo Almoço via "venda")
  manage_sales: [
...
export default function Sidebar({ open, setOpen }) {
  ...
  const menuGroups = useMemo(
    () => [
      ...
      /** Novo grupo: Almoço (quatro páginas) */
      {
        title: "Almoço",
        items: [
                  ...
                  return (
                    <li key={item.path}>
                      ...
                      >
                        ...
                        {!open && !isMobile && (
                          <div className="absolute left-full top-1/2  ml-2 px-2 py-1 bg-gray-800/95 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition pointer-events-none border border-white/10">
                            {item.label}
                          </div>
                        )}
                      </NavLink>
