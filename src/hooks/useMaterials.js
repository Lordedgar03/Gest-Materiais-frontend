...
export function useMaterials() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const catsRaw = pickArray(catRes).map((c) => ({
          cat_id: c.cat_id,
          cat_nome: c.cat_nome,
        }));
        ...
        const visibleCats = isAdmin
          ? catsRaw
          : catsRaw.filter((c) =>
              allowedCategoryIds.includes(Number(c.cat_id))
            );
        const visibleTypes = isAdmin
          ? typesRaw
          : typesRaw.filter((t) =>
              allowedCategoryIds.includes(Number(t.tipo_fk_categoria))
            );
        const visibleMaterials = isAdmin
          ? matsRaw
          : matsRaw.filter((m) =>
              visibleTypes.some(
                (t) => Number(t.tipo_id) === Number(m.mat_fk_tipo)
              )
            );
  ...
  }, []); // canView e outros já estão “fixos” pela regra do teu projeto
  ...
  const filteredMaterials = useMemo(() => {
    ...
    return materials.filter((m) => {
      ...
      const byType =
        !selectedType || String(m.mat_fk_tipo) === String(selectedType);
      ...
      const byCategory =
        !selectedCategory ||
        (typeObj &&
          String(typeObj.tipo_fk_categoria) === String(selectedCategory));
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.mat_fk_tipo) errs.mat_fk_tipo = "Tipo é obrigatório.";
      ...
      errs.mat_fk_tipo = "Sem permissão para este tipo.";

    if (!String(formData.mat_localizacao || "").trim())
      errs.mat_localizacao = "Localização é obrigatória.";

    if (asNumber(formData.mat_preco, -1) < 0)
      errs.mat_preco = "Preço inválido.";
  ...
  const openDeleteModal = (material, mode = "partial") => {
    ...
    setDeleteMode(mode);
    setDeleteQty(
      mode === "all" ? estoqueAtual : Math.max(1, Math.min(1, estoqueAtual))
    );
    setDeleteReason("");
    setDeleteErrors({});
    setDeleteOpen(true);
  ...
  const validateDelete = () => {
    ...
    if (!Number.isFinite(q) || q <= 0) errs.qty = "Quantidade inválida.";
      ...
      errs.qty = `Maior que o estoque disponível (${estoqueAtual}).`;

    if (!deleteReason || deleteReason.trim().length < 3)
      errs.reason = "Descreva o motivo.";
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await api.delete(`/materiais/${id}`, {
        data: {
          quantidade: asNumber(deleteQty),
          descricao: deleteReason.trim(),
        },
      });
      closeDeleteModal();
      await refetchMaterials();
  ...
  const mapHeader = (h) => {
    ...
    if (["nome", "mat_nome", "material", "produto"].includes(k))
      return "mat_nome";
    if (["descricao", "descrição", "mat_descricao", "desc"].includes(k))
      return "mat_descricao";
    if (["preco", "preço", "mat_preco", "price"].includes(k))
      return "mat_preco";
    if (
      [
        "quantidade",
    ...
    if (
      ...
      return "mat_estoque_minimo";

    // tipo: id ou nome
    if (["tipo", "mat_fk_tipo", "tipo_id"].includes(k)) return "mat_fk_tipo";
    if (["tipo_nome", "tiponome", "tipo name"].includes(k)) return "tipo_nome";

    if (["localizacao", "localização", "local", "mat_localizacao"].includes(k))
      return "mat_localizacao";
    if (["vendavel", "vendável", "mat_vendavel"].includes(k))
      return "mat_vendavel";
    if (["consumivel", "consumível", "mat_consumivel"].includes(k))
      return "mat_consumivel";

    return null;
  };

  const parseVendavel = (v, def = "SIM") => {
    const s = normKey(v);
    if (["sim", "s", "1", "true", "yes"].includes(s)) return "SIM";
    if (["nao", "não", "n", "0", "false", "no"].includes(s)) return "NAO";
    return def;
  };

  const parseConsumivel = (v, def = "não") => {
    const s = normKey(v);
    if (["sim", "s", "1", "true", "yes"].includes(s)) return "sim";
    if (["nao", "não", "n", "0", "false", "no"].includes(s)) return "não";
    return def;
  };

  const importMaterialsFromExcel = async (file) => {
    if (!file) return;
    ...
    try {
      ...
      if (!ws) throw new Error("Ficheiro sem folhas (sheets).");

      // header row
      const headerRow = ws.getRow(1);
      const headerMap = {}; // colIndex -> fieldKey
      headerRow.eachCell((cell, colNumber) => {
        const key = mapHeader(cell?.value);
        if (key) headerMap[colNumber] = key;
      });

      const hasNome = Object.values(headerMap).includes("mat_nome");
      const hasTipo =
        Object.values(headerMap).includes("mat_fk_tipo") ||
        Object.values(headerMap).includes("tipo_nome");
      const hasLoc = Object.values(headerMap).includes("mat_localizacao");

      if (!hasNome || !hasTipo || !hasLoc) {
        throw new Error(
          "Cabeçalhos inválidos. Precisa de: Nome + Tipo (id ou nome) + Localização."
        );
      }

      const typeByName = new Map(types.map((t) => [normKey(t.tipo_nome), t]));
      const toCreate = [];
      const warnings = [];
      ...
      ws.eachRow((row, rowNumber) => {
        ...
        const obj = {};
        Object.entries(headerMap).forEach(([col, field]) => {
          obj[field] = row.getCell(Number(col))?.value;
        });

        const nome = norm(obj.mat_nome);
        const localizacao = norm(obj.mat_localizacao);

        let tipoId = obj.mat_fk_tipo != null ? asNumber(obj.mat_fk_tipo, 0) : 0;
        if (!tipoId && obj.tipo_nome) {
          const t = typeByName.get(normKey(obj.tipo_nome));
          tipoId = t ? asNumber(t.tipo_id, 0) : 0;
        }

        if (!nome || !localizacao || !tipoId) {
          warnings.push(
            `Linha ${rowNumber}: falta Nome/Localização/Tipo válido.`
          );
          return;
        }

        if (!canManageType(tipoId)) {
          warnings.push(
            `Linha ${rowNumber}: sem permissão para o Tipo ${tipoId}.`
          );
          return;
        }
        ...
        toCreate.push({
          mat_nome: nome,
          mat_descricao: norm(obj.mat_descricao),
          mat_preco: asNumber(obj.mat_preco, 0),
          mat_quantidade_estoque: asNumber(obj.mat_quantidade_estoque, 0),
          mat_estoque_minimo: asNumber(obj.mat_estoque_minimo, 3),
          mat_fk_tipo: tipoId,
          mat_localizacao: localizacao,
          mat_vendavel: parseVendavel(obj.mat_vendavel, "SIM"),
          mat_consumivel: parseConsumivel(obj.mat_consumivel, "não"),
        });
      });
      ...
      // envia um a um (compatível com teu backend atual)
      ...
      if (warnings.length) {
        console.warn("Importação com avisos:\n" + warnings.join("\n"));
        setError(
          `Importado com avisos (${warnings.length}). Ex.: ${warnings
            .slice(0, 3)
      ...
      console.error(e);
      setError(e?.message || "Erro ao importar Excel.");
      throw e; // para o UI poder mostrar toast de erro
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportMaterialsPdf = async () => {
    try {
      ...
      const nowTxt = new Date().toLocaleString("pt-PT");

      doc.setFontSize(14);
      doc.text("Relatório de Materiais", 14, 12);
      doc.setFontSize(9);
      doc.text(`Gerado em: ${nowTxt}`, 14, 18);

      const rows = filteredMaterials.map((m, i) => {
        const tipo =
          types.find((t) => String(t.tipo_id) === String(m.mat_fk_tipo))
            ?.tipo_nome ?? "-";
        const low =
          Number(m.mat_quantidade_estoque) < Number(m.mat_estoque_minimo);
        ...
        return [
          ...
          tipo,
          m.mat_localizacao ?? "",
          Number(m.mat_preco || 0).toLocaleString("pt-PT", {
            style: "currency",
            currency: "STN",
          }),
          String(m.mat_quantidade_estoque ?? 0),
          String(m.mat_estoque_minimo ?? 0),
          m.mat_vendavel === "SIM" ? "Sim" : "Não",
          String(m.mat_consumivel).toLowerCase() === "sim" ? "Sim" : "Não",
          low ? "BAIXO" : "OK",
        ];
      });
      ...
      doc.save(`materiais_${Date.now()}.pdf`);
    } catch (e) {
      console.error(e);
  ...
  return {
    ...
    allowedTypeIds: types
      .filter(
        (t) =>
          isAdmin || allowedCategoryIds.includes(Number(t.tipo_fk_categoria))
      )
      .map((t) => t.tipo_id),
